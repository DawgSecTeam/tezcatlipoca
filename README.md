...
services and deploy misconfigs. Team subnets are isolated (no uplink), so the scoring engine
is the only thing with a NIC on every team's network — it's also where nakon actually runs.
...
| `boxes_per_team` | JSON list of boxes cloned per team. `template` must match a tagged Proxmox template name; `disk_gb` is optional (omit to keep the template's own disk size) |
...
Box `name` matters: anything starting with `ssh` gets an SSH check + credlist wired up
automatically, `web`/`dns` get matching check types. Other names get no check — fine for a
box that's just an attack target with no scored service.
...
and `terraform apply` directly — only the new bridge + boxes are created.
...
| Quotient panics on startup re: credlist | A box check references a credlist that was never staged | Should be automatic for `ssh` boxes — check `config/credlists/` on the scoring engine |
...
| `apt-get install` fails with `Temporary failure resolving` on fresh boxes | Cloud-init's `dns.servers` is silently ignored on Debian with static IPs, leaving `/etc/resolv.conf` empty. Step D patches this with a `systemd-resolved` drop-in, but if Step D's SSH connection to a box failed silently, that box missed the fix | Taint `null_resource.orchestrate` and re-run `terraform apply`. To debug: SSH through the scoring engine to the affected box and check `cat /etc/systemd/resolved.conf.d/upstream.conf` and `cat /etc/resolv.conf` |
