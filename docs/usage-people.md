...
generates nakon's machine + vuln list itself (`nakon.catalog.randomize`, imported in-process),
...
| nakon: MySQL access denied / connection refused | vulndb unreachable from your laptop when generating the machine list, or `nakon/.env` creds/db name don't match actual grants | Confirm reachability, check `SHOW GRANTS`, fix `nakon/.env` |
